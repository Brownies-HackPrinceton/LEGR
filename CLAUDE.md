# CLAUDE.md — Agent Context for Flux

> This file is the primary context for any AI coding agent (Claude Code, Cursor, etc.) working on this repo. Read SPEC.md and BUILD_PLAN.md in addition to this file before making changes.
...
3. **Reasoning > rules.** Don't regex-match merchant names to categorize. Use Dedalus with Claude Sonnet 4.5 for anything that has ambiguity. The orchestrator's job is to REASON about routing.
...
| Task | Model | Why |
|---|---|---|
| Orchestrator routing decisions | Claude Haiku 4.5 | Fast, cheap, good at structured output |
| AI-Spend Agent reasoning | Claude Sonnet 4.5 | Needs to actually reason about prompt patterns |
| SaaS Discovery (invoice parsing) | Gemini 2.5 Flash | Cheap OCR + structured extraction |
| Usage Signals (seat inference) | Claude Haiku 4.5 | Simple heuristics + light reasoning |
| Negotiation Agent (email drafting) | Claude Sonnet 4.5 | Tone matters, reasoning about BATNA |
| Compliance Agent | Claude Haiku 4.5 | Policy checking is near-deterministic |
...
- **LLMs:** Anthropic SDK (Claude), Google SDK (Gemini)
